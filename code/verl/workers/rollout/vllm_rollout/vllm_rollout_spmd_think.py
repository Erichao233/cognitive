    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        while True:
            ...
            if self.rag_enabled and self.rag_index is not None and rag_queries:
                ...
                for i, results in enumerate(rag_results):
                    ...
                    messagess_todo[i] = self._inject_rag(messagess_todo[i], rag_text)
                    sample_idx = todo[i]
        ...
        for i in range(batch_size*n):
            for _ in range(dialogue_turns[i]): 
                ...
                expanded_rag_query.append(rag_meta["rag_query"][i])
                expanded_rag_card_ids.append(rag_meta["rag_card_ids"][i])
                expanded_rag_scores.append(rag_meta["rag_scores"][i])
        ...
        if self.rag_enabled:
            non_tensor_batch.update({
                'rag_query': to_1d_np_array(expanded_rag_query),
