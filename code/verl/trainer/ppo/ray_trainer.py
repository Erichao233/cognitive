...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        response_length = responses.size(-1)
        attention_mask = data.batch['attention_mask']
        if 'generation_mask' in data.batch.keys():
            generation_mask = data.batch['generation_mask']
            response_mask = (attention_mask * generation_mask)[:, -response_length:]
        else:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        ...
                        if self.config.trainer.save_rollout:
                            rollout_root = os.getenv("ROLLOUT_LOG_DIR", "./rollout_logs")
                            rollout_dir = os.path.join(rollout_root, f"{self.config.trainer.experiment_name}", f'global_step_{self.global_steps}')
